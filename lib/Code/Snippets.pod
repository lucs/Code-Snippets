...
Main program:
    ...
    my $CS = Code::Snippets.build(
        ...
        snip-id_rx  => / <after ^^ "# ID:" \s+> \S+ /,
    );

A corresponding snippets file:

    This text precedes any snippet.
    ...
    This text will be ignored, it's between two snippets.
...
When learning a programming language, I often try out short code
snippets and like to keep them around for reference. But I find it
...
Each snippet is identified by a string

made up of the non-whitespace characters immediately following the
snippet marker 

This expression holds the snippet alias
...
Any snippet named "SKIP" is special and the corresponding snippet
...
That would extract the snippet to a file named 'eg01' in the 
    ...
    $CS.extract: :content(sub ($a, $b, $c) {$b}), @snal-globs;
