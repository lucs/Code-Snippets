...
they have a special meaning:
    ...
    . : Don't append $self.file-ext to the snippet file name.
...
=head1 Snip alias expressions

The ｢◑｣ represents a string of at most one each of ｢.｣ and ｢-｣, in any
order, and may be empty, where the symbols mean:

    ｢.｣ : No automatic file extension.
    ｢-｣ : ID line and header not placed in extract.

So, one of < . - .- -. >, or empty.

        Skip the snippet completely.
    SKIP

    ｢Σ｣ represents the snippet alias, ｢M｣, the name of what will be
    the main file name (if not ｢Σ｣), also marked with ｢*｣, the other
    letters, arbitrary file (or sometimes, directory) names.

        Single file in top level.
    ◑Σ        * ⋯/Σ

        Multiple files in top level.
    ◑Σ:M      * ⋯/M
    ◑Σ::a       ⋯/a

        Single file in subdir.
    ◑Σ/       * ⋯/Σ/Σ
...
Example snippet aliases and their corresponding file names, given that
the default file extension is ｢raku｣, with the main file indicated by
｢*｣:
